...
class FirebaseSyncManager(
    ...
    suspend fun updateExam(exam: ExamEntity): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val db = firestore
            if (db != null) {
                val docId = if (exam.firebaseId.isNotEmpty()) exam.firebaseId else db.collection("exams").document().id // Fallback if no firebaseId
    ...
    suspend fun fetchAllExams() = withContext(Dispatchers.IO) {
        try {
            ...
            // Keep local un-synced exams? For now simple implementation: full replace
