...
// 1. Fix duration calculation & stacking in processPurchaseRequest
const processPurchaseRegex = /const durationMs = 365 \* 24 \* 60 \* 60 \* 1000;\s*const entitlementMap = \{[\s\S]*?updatedAt: now\s*\};/;
...
const replacement = `
        ...
        if (!plansQuery.empty) {
           ...
           functions.logger.warn("Purchased product ID not found in plans collection:", productId);
        }
...
`;
code = code.replace(processPurchaseRegex, replacement.trim());

// 2. Fix `grantPlanToUser` to be a secure callable function.
// Wait, is there a `grantPlanToUser` function already?
